...
async function main() {
  ...
  try {
    ...
    await db.class.create({ data: { id: ids.class, name: "Lớp verify", level: "BASIC", schedule: "Thứ 2" } });
    ...
    await db.exam.create({ data: { id: ids.exam, title: "Đề verify", examFileUrl: "", status: "PUBLISHED", examLinks: { create: { classId: ids.class } }, questions: { create: { number: 1, content: "Câu 1", options: ["A", "B"], correctAnswer: "B" } } } });
    ...
    console.log("✓ Learning management: enrollment, nội dung theo lớp, version tài liệu và autosave transaction đều đạt.");
  } finally {

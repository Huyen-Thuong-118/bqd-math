...
function normalize(answer: string, type: "MULTIPLE_CHOICE" | "TRUE_FALSE" | "SHORT_ANSWER") {
  let result = answer.trim().toUpperCase().replace(/\s+/g, " ");
  if (type === "TRUE_FALSE") result = result.replace(/Đ/g, "D").replace(/[;|/]/g, ",").replace(/\s*,\s*/g, ",");
  if (type === "SHORT_ANSWER") result = result.replace(/\s/g, "").replace(",", ".");
  return result;
}
...
export async function createReviewQuestion(formData: FormData): Promise<Result> {
  try {
    ...
    const { classIds, ...data } = parsed.data; if (!(await validateRelations(data.chapterId, classIds))) return { success: false, error: "Chương hoặc lớp không tồn tại/lớp đã lưu trữ." };
    await db.reviewQuestion.create({ data: { ...data, classLinks: { create: classIds.map((classId) => ({ classId })) } } }); refresh(data.chapterId); return { success: true };
  } catch (error) { console.error("createReviewQuestion thất bại:", error); return { success: false, error: "Không thể tạo câu hỏi." }; }
}
...
export async function answerReviewQuestion(questionId: string, selectedAnswer: string): Promise<ReviewAnswerResult> {
  try {
    ...
    const question = await db.reviewQuestion.findFirst({
      ...
      select: { id: true, type: true, correctAnswer: true, textSolution: true, solutionImageUrl: true, videoUid: true, videoUrl: true, showSolution: true },
    });

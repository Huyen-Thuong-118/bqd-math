...
import { FilePlus2, FolderPlus, Save, UploadCloud } from "lucide-react";
...
export function AdminDocumentsManager({ folders, classes, documents, directUpload }: { folders: FolderOption[]; classes: ClassOption[]; documents: DocumentItem[]; directUpload: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <ResponsiveTreePanel title="Cây thư mục tài liệu" triggerLabel="Mở cây thư mục tài liệu">
      <div className="min-w-0 max-w-full overflow-hidden rounded-3xl border border-navy-100 bg-white p-5">
        <h2 className="font-semibold text-navy-600">Cây thư mục</h2>
        <div className="mt-3">
          <ResourceTree
            folders={folders}
            resources={documents.map((item) => ({ id: item.id, title: item.title, folderId: item.folderId, position: item.position, meta: item.fileName }))}

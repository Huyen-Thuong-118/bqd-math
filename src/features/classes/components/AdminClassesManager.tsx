  ...
  return (
    <details className="mt-3 border-t border-navy-50 pt-3">
      ...
      <div className="mt-3 space-y-3">
        <div className="relative"><Search className="absolute left-3 top-3.5 size-4 text-navy-300" /><input className={`${input} pl-9`} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm theo tên hoặc email…" /></div>
        ...
        <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
          {visibleStudents.map((student) => <label key={student.id} className="flex items-center gap-2 rounded-xl bg-pastel-50 px-3 py-2 text-xs text-navy-500"><input type="checkbox" checked={selected.has(student.id)} onChange={(event) => { const checked = event.target.checked; setSelected((current) => { const next = new Set(current); if (checked) next.add(student.id); else next.delete(student.id); return next; }); }} /><span><strong>{student.name}</strong><span className="block text-navy-300">{student.email}</span></span></label>)}

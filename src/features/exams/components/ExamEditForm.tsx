    ...
    <form action={submit} className="space-y-6">
      ...
      <fieldset disabled={pending} className="space-y-6 disabled:opacity-70">
        ...
        <section className="grid gap-4 rounded-3xl border border-navy-100 bg-white p-5 sm:grid-cols-2">
          ...
          {!isForever && <><label className="text-sm font-medium text-navy-500">Mở từ (giờ Việt Nam)<input name="availableFrom" required type="datetime-local" defaultValue={exam.availableFrom} className={input} /></label><label className="text-sm font-medium text-navy-500">Đóng lúc (giờ Việt Nam)<input name="availableTo" required type="datetime-local" defaultValue={exam.availableTo} className={input} /></label></>}

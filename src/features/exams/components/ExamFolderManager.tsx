...
export function ExamFolderManager({ folders, exams, mode = "tree" }: { folders: FolderOption[]; exams: TreeResource[]; mode?: "tree" | "grid" }) {
  ...
  function createFolderFromGrid(name: string, parentId: string | null) {
    ...
    run(() => createExamFolder(data));
  }

  const panel = (
      <section className="min-w-0 max-w-full rounded-3xl border border-navy-100 bg-white p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><h2 className="font-semibold text-navy-600">{mode === "grid" ? "Kho đề thi" : "Cây thư mục đề thi"}</h2><p className="mt-1 text-xs text-navy-300">{mode === "grid" ? "Mở thư mục, kéo thả và sắp xếp như một ổ đĩa." : "Kéo thả đề và thư mục để sắp xếp trực quan."}</p></div>
        ...
        <div className="mt-4">
          <ResourceTree
            ...
            onCopyFolder={(id, parentId) => run(() => copyExamFolder(id, parentId))}
            onCopyResource={(id, folderId) => run(() => copyExam(id, folderId))}
          />
        </div>
      </section>
  );

  if (mode === "grid") return panel;
  return <ResponsiveTreePanel title="Cây thư mục đề thi" triggerLabel="Mở cây thư mục đề thi">{panel}</ResponsiveTreePanel>;
}

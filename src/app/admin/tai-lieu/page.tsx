...
function flattenFolders(folders: { id: string; name: string; parentId: string | null; _count: { children: number; documents: number } }[]) {
  ...
  return folders.map((folder) => ({ id: folder.id, name: folder.name, label: label(folder), count: folder._count.children + folder._count.documents })).sort((a, b) => a.label.localeCompare(b.label, "vi"));
}

export default async function AdminDocumentsPage() {
  const [folderRows, classes, rows] = await Promise.all([
    db.folder.findMany({ select: { id: true, name: true, parentId: true, _count: { select: { children: true, documents: true } } } }),
    db.class.findMany({ where: { status: "ACTIVE" }, select: { id: true, name: true }, orderBy: { name: "asc" } }),
    db.document.findMany({
      select: { id: true, title: true, fileName: true, contentType: true, folderId: true, allowDownload: true, showAnswer: true, answerUrl: true, updateCount: true, createdAt: true, classLinks: { select: { classId: true, class: { select: { name: true } } } }, versions: { select: { id: true, version: true, fileName: true, createdAt: true }, orderBy: { version: "desc" } } },

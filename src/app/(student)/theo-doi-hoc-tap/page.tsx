...
import { getMyStudentProgress, type ProgressFilters } from "@/features/progress/queries";

export const metadata: Metadata = { title: "Theo dõi học tập | BQD Math" };
...
function pageHref(filters: ProgressFilters, page: number) {
...
export default async function MyProgressPage({
  ...
  const data = await getMyStudentProgress(filters);
  if (!data) return null;
  const summary = data.summary;

  return (
    <section className="space-y-6">
      <div className="rounded-3xl bg-linear-to-br from-navy-600 to-navy-800 p-6 text-white">
        <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-pastel-200">
        ...
        <p className="mt-2 text-sm text-pastel-100">Mã học sinh: {data.student.studentCode ?? "Chưa có mã"}</p>
      ...
      {data.trend.length > 0 && (
        <section className="rounded-3xl border border-navy-100 bg-white p-5">
          ...
          <p className="mt-1 text-xs text-navy-300">Cột xanh: điểm đề trung bình. Cột vàng: tỉ lệ câu ôn đúng.</p>
